...
标题栏**左侧**为品牌：小鹿标记 + 「乔巴」（≤480px 只留标记）。标记是**原创**驯鹿（圆脸、鹿角、**戴耳机**；不是尾田笔下的角色形象，名字可用、形象不能照抄），几何定义在 `src/lib/brand/logo.ts` 一处，标题栏、favicon、manifest、PNG 图标全部由它生成。

**彩蛋**：点标题栏的小鹿或「乔巴」二字 → 弹出「我是乔巴」（`AboutChopper.svelte`）。内容只讲一件事：乔巴是作者最喜欢的动漫《海贼王》里的角色，是驯鹿也有一半是人，所以动物和人说的话都能听懂，正适合当翻译。配图用的是**应用自己的小鹿**（跟着设置里选的图标配色走），不是尾田笔下的角色形象；Esc / 点遮罩 / 按钮均可关闭。

图标文件**不要手改**：`npm run icons`（`scripts/make-icons.mjs`）从上面的几何渲染出 `static/icon.svg` 与 192 / 512 / maskable 512 / apple-touch-icon(180) 四个 PNG，并顺带量出"图案最远点距中心的半径"，确认它仍在 maskable 允许被裁的 80% 圆内。PNG 的存在是因为 Android 安装要 PNG、iOS 的 `apple-touch-icon` 从不支持 SVG（缺了它 iPhone 会把页面截图当图标）。
...
| 设置项（人话） | 气泡说明 | 默认 |
|---|---|---|
| 应用图标 | 装到手机/桌面时用的图标；已装过的要删掉重新添加才会换 | 原色小鹿（6 款可选：原色/芥末黄/墨蓝/海蓝/松绿/琥珀） |
